...
# get Runelite window
USERNAME = 'jonslildoner' # CHANGE ME!
...
pg.press('f3'), sleep(0.6) # select combat options
try:
...
while True:
	...
	if any(findColorMiddle(window,NPC)):
		...
		counter+=1
		sleep(3) # yaks + xp globes
		#sleep(5) # experiments + xp globes
	else:
		continue
	# check to see if still in combat
	if pg.locateOnWindow('images\\combat4.png',title,confidence=.9):
		while pg.locateOnWindow('images\\combat4.png',title,confidence=.9):
			sleep(0.06)
			pg.locateOnWindow('images\\combat4.png',title,confidence=.9)
	else:
		continue