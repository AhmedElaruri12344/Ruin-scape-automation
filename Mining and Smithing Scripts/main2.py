...
def isInventoryFull():
    ...
    if result.__len__()>0:
        center = ( (maxLoc[0] + width/2, maxLoc[1] + height/2 ) )
        print(center)
        lastSlot = center[0], center[1] - 47    #38
        print(lastSlot)
        if(pyautogui.screenshot().getpixel(lastSlot) == (62, 53, 41)):
            return False
        else:
            return True
...
def miningGuildBankRock():
    ...
    while True:
        #logoutHandler()
        if isInventoryFull():
            #findObjectCenter(tile) # run to tile
            #time.sleep(5.8)
            findObjectCenter(bank) # run to bank
            time.sleep(4)
            depositAll()
            findObjectCenter(tile) # run back to tile
            time.sleep(4)
        else:
            findObjectCenter(rock, True)
            time.sleep(2.4) # delay before clicking another rock
